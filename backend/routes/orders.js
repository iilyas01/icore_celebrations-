...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    // Check at least 1 package exists
    const [packages] = await db.query(
      'SELECT * FROM plan_packages WHERE plan_id = ?', [planId]
    );
    if (packages.length === 0)
      return res.status(400).json({ error: 'Please add at least one package.' });

    // Check at least 1 service exists
    const [services] = await db.query(
      'SELECT * FROM plan_services WHERE plan_id = ?', [planId]
    );
    if (services.length === 0)
      return res.status(400).json({ error: 'Please add at least one service.' });

    // Check if order already exists for this plan
    const [existingOrders] = await db.query(
      'SELECT * FROM orders WHERE plan_id = ?', [planId]
    );
