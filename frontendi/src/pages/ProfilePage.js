...
const ProfilePage = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'confirmed': return <CheckCircle size={18} className="text-green-500" />;
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {orders.map(order => (
                <div
                  key={order.order_id}
                  className="border border-slate-100 rounded-2xl p-6 hover:border-amber-200 transition-colors"
                >
                  ...
                  {order.order_status === 'cancelled' && (
                    <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-3">
                      <p className="font-nunito text-red-700 text-sm">
                        ❌ Your order was cancelled. Please contact us for more information.
                      </p>
                    </div>
                  )}
                </div>
